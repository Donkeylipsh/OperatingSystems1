 ...
 * Date: 6/10/2018
 * Assignment: OTP
 * Description: otp_enc_d.c is a simple server that accepts a string of characters and a key, and returns an
 * 	encrypted string. otp_enc_d demonstrates forking a new process for each new connection, without waiting 
 * 	the child process to complete. The child process will perform the encryption and return the new
...
int main(int argc, char *argv[])
{
	...
	//socklen_t sizeOfPeerInfo;
	//int peerResponse = getpeername(establishedConnectionFD, (struct sockaddr *)&connectedPeer, &sizeOfPeerInfo);
	//printf("Connected with peer: %i\n", ntohs(connectedPeer.sin_port));
	...
	spawnpid = fork();
        if(spawnpid == 0){   
            ...
            /*******************************************************************************
             * Get the size of the plain tex tinput string from the client and display it
             * *****************************************************************************/
	    ...
	    /*******************************************************************************
 	     ...
 	     *******************************************************************************/
	    //printf("Listening for %d characters of plain text...\n", msgSize);
	    while(charsLeft > 0){
		// Clear the buffer
	    ...
	    //printf("SERVER: Total key processed, moving to encryption\n");
	    ...
	    //printf("SERVER: Starting encryption...\n");
	...
	// Don't wait for child to complete, immediately return to listening for new connections
